...
%macro HCUP_MARKT_MERGE(state,year);
	PROC SQL;
		...
		SELECT A.DSHOSPID,
				...
				B.MNAME,
				B.MLOCZIP
		FROM &state..&state._sidc_&year._ahal AS A LEFT JOIN AHA.PUBAS11 AS B
		ON A.AHAID=B.ID;
	QUIT;

	DATA work.HCUP_&state.&year._v1(keep=age pstate pcity zip dshospid hospst atype hcup_ed mdc newdrg drg pr1 pr2 pay1) ;
...
%MACRO HCUP_MARKT_FORMAT(STATE,YEAR);
	...
	DATA markt.HCUP_&state.&year. work.HCUP_&state.&year._excel(drop=atype HCUP_ED pay1 DRG) ;
		length zip atypedesc pay1desc pstate$ 30;
		length pr1 $ 10;
		retain AGE PCITY PSTATE ZIP ATYPE ATYPEdesc MDC NEWDRG DRG PR1 PAY1 PAY1desc HOSPST DSHOSPID AHAID MNAME MLOCZIP;
